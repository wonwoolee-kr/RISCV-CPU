...
In this project, instruction decoding uses named constants defined in `rv32i_pkg.sv`.
...
Using named constants is better than writing raw binary values directly inside the Decoder.
...
This makes the RTL easier to read and easier to maintain.

---

## Boolean Flag Naming

A 1-bit signal that represents a true/false condition is often named using prefixes such as `is_` or `has_`.
...
```text
is_r_type_o  → this instruction is R-type
is_load_o    → this instruction is a load instruction
is_branch_o  → this instruction is a branch instruction
instr_valid_o → this instruction is supported and valid
```

This naming style makes it clear that the signal is a boolean flag.

Common naming patterns:

| Pattern | Meaning |
|---|---|
| `is_*` | Indicates a category or state |
| `has_*` | Indicates the presence of a condition |
| `*_valid` | Indicates valid data or valid instruction |
| `*_en` | Enable signal |
| `*_sel` | Select signal for a mux |
| `*_addr` | Address signal |
| `*_data` | Data signal |

---

## Control Signal Naming

Control signals describe how datapath blocks should behave.

Examples from the Control Unit:
...
| Signal | Meaning |
|---|---|
| `reg_write_o` | Enables writing to the Register File |
| `alu_src_o` | Selects ALU operand B source |
| `mem_read_o` | Enables Data Memory read |
| `mem_write_o` | Enables Data Memory write |
| `branch_o` | Indicates branch instruction |
| `wb_sel_o` | Selects writeback data source |

Control signals are usually generated by combinational logic because they depend on the current decoded instruction.

---

## Safe Default Control Values

For control logic, it is important to use safe default values.
...
The default values disable side effects.

This is important because invalid or unsupported instructions should not accidentally modify architectural state.
...
This reduces the risk of unintended behavior.

---

## Side Effects in CPU Control Logic

A side effect means a change to architectural state.
...
Changing the Program Counter through branch or jump control
```

Control logic must be careful when enabling side effects.

For example, if an invalid instruction accidentally sets `reg_write_o = 1`, the CPU may corrupt a register.

If an invalid instruction accidentally sets `mem_write_o = 1`, the CPU may corrupt memory.

Therefore, `instr_valid_i` is used to ensure that side-effect control signals are disabled for unsupported instructions.

---

## Writeback Select Encoding

The writeback mux selects the data source that will be written to the Register File.

In this project, writeback select values are defined as:
...
This is more extensible than a simple `MemtoReg` signal.

For example, future instructions may require additional writeback sources:

```text
PC + 4 for JAL/JALR
Immediate value for LUI
PC + immediate for AUIPC
```

In that case, `wb_sel_o` can be extended more naturally than a 1-bit `MemtoReg`.

---

## Decoder and Control Unit Separation

In a simple textbook datapath, the Control block may appear as one block.

In this project, the logic is separated into two modules.
...
This separation makes the design easier to test.

Each module can have its own self-checking testbench:
