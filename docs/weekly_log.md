...
## Week 1 - Control Unit Module and Testbench

### Completed

- Implemented `control_unit.sv` using synthesizable SystemVerilog.
- Added writeback select parameters to `rv32i_pkg.sv`.
- Generated datapath control signals:
...
- Supported control behavior for:
  - R-type
  - OP-IMM
  - LOAD
  - STORE
  - BRANCH
  - Invalid instruction
- Created a self-checking testbench `tb_control_unit.sv`.
- Created simulation script:
...
Control unit simulation passed.

Result summary:

- PASS: 7
- FAIL: 0
- Final result: CONTROL UNIT TEST PASSED

### Notes

The control unit receives first-level decode flags from the decoder and generates datapath control signals.

The decoder identifies the instruction category, while the control unit determines how the datapath should move data.

The actual branch decision will later be generated by combining `branch_o` with the ALU zero flag.

### Next Step

Start integrating the single-cycle datapath by connecting the decoder, control unit, register file, immediate generator, and ALU.